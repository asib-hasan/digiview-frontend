<template>
  <div class="news-detail-page bg-white min-h-screen pt-12 md:pt-16 pb-20">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
        ...
        <div class="lg:col-span-2">
          ...
          <div class="mt-16 pt-8 border-t border-slate-100">
            <NuxtLink to="/news" class="inline-flex items-center gap-3 px-8 py-4 bg-slate-50 hover:bg-[#e32727] text-slate-700 hover:text-white font-bold uppercase tracking-widest text-sm rounded-xl transition-colors duration-300">
    ...
    <div v-else class="container mx-auto px-4 py-40 text-center">
      ...
      <p class="text-slate-600 mb-8">The news article you are looking for does not exist.</p>
      <NuxtLink to="/news" class="inline-flex items-center gap-3 px-8 py-3 bg-[#e32727] text-white font-bold rounded-lg hover:bg-slate-900 transition-colors">
        View All Articles
      </NuxtLink>
    </div>
  </div>
...
const { data: blogResponse } = await useAsyncData(`news-${route.params.slug}`, () => 
...
useSeoMeta({
  ...
  description: computed(() => blog.value?.excerpt || 'News article details.'),
})
</script>
